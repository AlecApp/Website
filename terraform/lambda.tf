...
resource "null_resource" "setup_db" {
  provisioner "local-exec" {
    command = "sed -e s/MASTER_USER/${aws_ssm_parameter.master_username.value}/g -e s/MASTER_PASSWORD/${aws_ssm_parameter.master_password.value}/g -e s/MASTER_USER/${aws_rds_cluster.db.database_name}/g ../setup_db/setup_db.py >> setup_db.py"
  }
}
